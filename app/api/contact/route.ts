...
export async function POST(req: NextRequest) {
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: 'noreply@resend.dev', to: 'matt@amplifyforlawyers.com', subject: `New Contact — ${name}`, html: `<p><b>Name:</b> ${name}</p><p><b>Phone:</b> ${phone}</p><p><b>Email:</b> ${email}</p><p><b>Message:</b> ${message}</p>` })
  })
